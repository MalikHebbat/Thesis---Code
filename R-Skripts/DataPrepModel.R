...
##graph course NC Rate grouped by cluster
...
## Correlations with NC Rate

l1 <-  lares::corr_var(OSH_viet, # name of dataset
                       factory_cp_level_nc_rate, # name of variable to focus on
...
l2 <- lares::corr_var(OSH_indo, # name of dataset
                      factory_cp_level_nc_rate, # name of variable to focus on
                      top = 20, # display top 5 correlations,
                      max_pvalue=0.05,
                      save=T
...
l3 <- lares::corr_var(OSH_jord, # name of dataset
                      factory_cp_level_nc_rate, # name of variable to focus on
                      top = 20, # display top 5 correlations,
                      max_pvalue=0.05,
                      save=T,
...
##decompose plot
agg <-data.frame("Vietnam" = OSH_viet$factory_cp_level_nc_rate,"Indonesia" =OSH_indo$factory_cp_level_nc_rate,"Jordan"=OSH_jord$factory_cp_level_nc_rate)


rownames(agg) <- rownames(OSH_viet)
agg <- xts(agg,order.by = as.yearqtr(rownames(agg)))

plot(stl(agg$Indonesia,s.window=7),main="Decomposition of FacCPLNC Indonesia")

plot(stl(agg$Vietnam,s.window=7),main="Decomposition of FacCPLNC Vietnam")

plot(stl(agg$Jordan,s.window=7),main="Decomposition of FacCPLNC Jordan")

###statistical test combined Ollech-Webel
seastests::combined_test(agg$Vietnam,freq = 4)
seastests::combined_test(agg$Indonesia,freq = 4)
seastests::combined_test(agg$Jord,freq = 4)
