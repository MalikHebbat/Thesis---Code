...
# This looks good no edits needed
...
for (country in countries) {
  ...
  Minimum_wage[[country]]  <-  Minimum_wage_1[[country]] %>% 
    ...
    select(-Minimum_wage.x, -Minimum_wage.y)
...
for (country in countries) {
  ...
  Unemployment_rate[[country]]  <-  Unemployment_rate_1[[country]] %>% 
    ...
    select(-Unemployment.x, -Unemployment.y)
...
### Population covered by at least one social protection benefit	# Suggest not to take it as we have very few data points
