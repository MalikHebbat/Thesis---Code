...
bw_tree1 <- rpart(factory_cp_level_nc_rate~.,data=tempDataForest$ximp)
bw_tree1_prune <- prune(bw_tree1,cp= bw_tree1$cptable[which.min(bw_tree1$cptable[,"xerror"]),"CP"])
fancyRpartPlot(bw_tree1_prune, uniform=TRUE,
main="Pruned Classification Tree")

rem1 <- c("total_cps","total_cp_nc","Cluster","HPI_rank")
temp <- tempDataForest$ximp[,!names(tempDataForest$ximp) %in% rem1]
bw_tree2 <- rpart(factory_cp_level_nc_rate~.,data=temp)
bw_tree2_prune <- prune(bw_tree2,cp= bw_tree1$cptable[which.min(bw_tree1$cptable[,"xerror"]),"CP"])
fancyRpartPlot(bw_tree2_prune, uniform=TRUE,
main="Pruned Classification Tree")
```
